...
from coordinator.master_coordinator_json import MasterCoordinatorJSON
...
mmc0: error -1 whilst initialising MMC card
    ...
    print_header("运行甲方MMC案例（JSON模式）")
    
    # 指定JSON数据目录
    json_dir = "/data/xuao/code_kg_search/linux_test"
    
    # 创建协调器（JSON模式）
    coordinator = MasterCoordinatorJSON(json_dir=json_dir)

...
from data.json_kg_interface import JSONGraphInterface
...
class MasterCoordinatorJSON:
    """主协调器 - JSON版本（不需要Neo4j）"""
    
    def __init__(self, json_dir: str = None, llm_client=None):
        """
        初始化协调器
        ...
        self.kg = JSONGraphInterface(json_dir)
        ...
        logger.success("协调器初始化完成（JSON模式）")
    ...
    def process_with_specific_functions(self, log_text: str, 
        ...
        Returns:
            分析结果
        """
        print_header("Bug定位分析流程（指定起止点，JSON模式）")
